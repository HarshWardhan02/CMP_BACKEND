...
export const generateHumanReadableID = async (idType:string, prefix:string) => {
    try {
      ...
      while (retry) {
            try {
                retry = false;
                retryCount += 1;
                generatedID = await upsertReadableID(idType, prefix, currentYear, currentMonth, initialNumber);
            } catch (exception) {
  ...
  const upsertReadableID = async (inputIdType:string, inputPrefix:string, currentYear:string, currentMonth:string, initialNumber:number) => {
    try {
  ...
  export const replaceSpecialCharacterByRegex = (inputGlobalSearchPattern: string): string => {
